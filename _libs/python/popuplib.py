...
langdata = {}
langdata['en'] = {'prev': "Previous", 'next': "Next", 'quit': "Exit", 'pending': "Pending"}  # by [NATO]Hunter
langdata['fr'] = {'prev': "Retour", 'next': "Suivant", 'quit': "Annuler", 'pending': "En attente"}  # by Nicolous
langdata['de'] = {'prev': "Zur\xc3\xbcck", 'next': "Weiter", 'quit': 'Schlie\xc3\x9fen',
                  'pending': "Wechselnd"}  # by [NATO]Hunter
...
class Popup_base(object):
    ...
    def __init__(self, pPopupid):
        ...
        self.timeoutsend = 0.01  # timeout in seconds for being in queue
    ...
    def i_userlang(self, userid):
        ...
        if user:
            ...
            if not self.cache.has_key(userlang):
                if self.cache.has_key(gLanguage):
                    return gLanguage
                else:
                    k = self.cache.keys()
                    if len(k) == 0:
    ...
    def i_endlangg(self, userlang):
        ...
        '''
        if not langdata.has_key(userlang):
            if langdata.has_key(gLanguage):
                return gLanguage
            else:
                k = langdata.keys()
                if len(k) == 0:
    ...
    def send(self, pUsers, prio=True):
        '''
...
class Popup_popup(Popup_base):
    ...
    def checklang(self, lang=None):
        ...
        if not self.lines.has_key(editlang):
    ...
    def information(self, listlevel):
        ...
        if listlevel >= 3:
            ...
            for item in self.menuvar:
                es.dbgmsg(0,
                          "    [" + str(item) + "]:  " + str(self.menuvar[item]) + '="' + str(self.menuval[item]) + '"')
        if listlevel >= 2:
        ...
        if isinstance(text, dict):
            ...
            for editlang in text:
                ...
                if (line == lines + 1 or self.isValidLine(line, editlang)):
                    ...
                    raise IndexError(
                        "Popuplib: Cannot insert line #%d to %s in language %s" % (line, self.name, editlang))
        else:
            if not lang:
                editlang = self.editlang
            else:
        ...
        if isinstance(text, dict):
            ...
            for editlang in text:
                ...
                if (self.isValidLine(line, editlang)):
                    ...
                    raise IndexError(
                        "Popuplib: Cannot modify line #%d in %s in language %s" % (line, self.name, editlang))
        else:
            if not lang:
                editlang = self.editlang
            else:
    ...
    def checkCache(self, userid):
        ...
        if self.cachemode != "user":
            es.dbgmsg(1, 'Popuplib: Not user-specific, returning cache="' + str(self.cache[userlang]) + '"' + str(
                type(self.cache[userlang])))
    ...
    def handleChoice(self, choice, userid):
        ...
        '''
        es.dbgmsg(1, "Popuplib: popup.handleChoice (" + str(choice) + str(type(choice)) + ", " + str(userid) + str(
            type(userid)) + ")")
...
class Popup_easymenu(Popup_base):
    ...
    def checklang(self, lang=None):
        ...
        if not self.options.has_key(editlang):
    ...
    def recache(self, users=[], pages=None):
        ...
        try:
            ...
            if userlang:
                if not pages:
                    pages = range(1, self.getPageNumber(len(self.options[userlang]), userlang) + 1)
    ...
    def pageformatend(self, page, total, language):
        ...
        if page > 1:
            build += str(self.c_pageformat).replace("%1", "8").replace("%2", langdata[language]['prev']) + "\n"
        elif not tight:
            build += " \n"
        if page < total:
            build += str(self.c_pageformat).replace("%1", "9").replace("%2", langdata[language]['next']) + "\n"
        elif not tight:
            build += " \n"
        if self.c_endsep:
    ...
    def checkCache(self, userid):
        ...
        if self.cachemode != "user":
            es.dbgmsg(1, 'Popuplib: Not user-specific, returning cache="' + str(self.cache[userlang][int(page)]) + str(
                type(self.cache[userlang][int(page)])) + '"')
    ...
    def handleChoice(self, choice, userid):
        ...
        handle menu choice as a result of client command
        '''
        es.dbgmsg(1, "Popuplib: easymenu.handleChoice (" + str(choice) + str(type(choice)) + ", " + str(userid) + str(
            type(userid)) + ")")
        userlang = self.i_userlang(int(userid))
        user = gUsers[userid]  # retrieve user object
        user.processing = self
...
class UserHolderClass(object):
    ...
    def __getitem__(self, userid):
        ...
        '''
        if not self.userdict.has_key(int(userid)):
    ...
    def __delitem__(self, userid):
        ...
        '''
        if self.userdict.has_key(int(userid)):
...
def close(pPopupid, pUsers):
    ...
    '''
    if not isinstance(pPopupid, basestring):
        for p in pPopupid:
            close(p, pUsers)
    else:
        try:
