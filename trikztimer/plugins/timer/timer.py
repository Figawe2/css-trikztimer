...
trikz = es.import_addon("trikztimer")
...
auth_zone = 1
admins = ConfigObj(es.getAddonPath("trikztimer") + "/admins.ini")
...
def load_client_pref():
            global client_pref
            dictPath = os.path.join( es.getAddonPath("trikztimer"), "/database/clientpref.db")
...
def displayRank(steamid, country=None):
    ...
    if not country:
        ...
        for item in mysql.query.fetchall("SELECT steamid, points FROM stats WHERE country='%s' ORDER by points DESC", (country,)):
            index += 1
            if item[0] == steamid:
                return index
    ...
    def timer_validate(self, steamid):
        if steamid in client_pref:
            player[steamid] = {'partner': None,
                               ...
                               'settings_speedmeter': 1,
                               'settings_macro': client_pref[steamid]["settings_macro"]}
        else: datahandle.client_validate(steamid)
...
def player_join_callback(data, data_pack):
    ...
    if bool(data):
        ...
        if data[4] == '0':
            ...
            TSQL.Query.execute("UPDATE stats SET name='%s', points_last_seen=points, ip='%s' WHERE steamid='%s'", (name, ip, steamid))
        else:
            TSQL.Query.execute("UPDATE stats SET name='%s', points_last_seen=points WHERE steamid='%s'",
                                (name, steamid))
        esc.msg('#snowPlayer:#245,61,0 %s #tomato[%s points] #245,0,61(%s/%s) #snowconnected from #245,0,61%s' % (name, data[1], rank, length, data[2]))
        earned = data[1] - data[3]
        if earned < 0:
            esc.tell(userid, '#255,0,0 ** NOTICE ** #0,255,0You have #255,0,0lost#0,255,0 %s points in the period you were inactive, because of new ranks!' % (earned))
        elif earned > 0:
            esc.tell(userid,
        ...
        TSQL.Query.execute("INSERT INTO stats (steamid, points, name, country, points_last_seen, ip) VALUES ('%s', 0, '%s', '%s', 0, '%s')" % (steamid, name, country, ip))
...
def load_zones(s_map):
    ...
    zones = {}
    commands = {}
    TSQL.Query.fetchall("SELECT location_1, location_2, location_3, location_4, restart_loc, id, type, name, tier, points, partner_needed FROM zones WHERE map='%s'", args=(s_map,), callback=load_zones_callback)
...
def ShowHudHint(userid):
            ...
            velocity = round(vector(float(es.getplayerprop(userid, 'CBasePlayer.localdata.m_vecVelocity[0]')), float(es.getplayerprop(userid, 'CBasePlayer.localdata.m_vecVelocity[1]'))).length(), 2)
            debug_mode = 1
                    ...
                    if player[steamid]["disabled"] == 0:
                        ...
                        if zones[id]["partner_needed"] == 1 and not CheckPartner(userid):
                            ...
                            if player[partner]["zone"] == "start" and player[partner]['id'] == id:
                                ...
                                hudhint(userid, '(%s)\n(REAL: %s)\n[%s Timer]\n \nMode: %s\nSpeed: %s' % (
                                TimeFormat((float(time.time()) - float(timer[timer_id]['time'])), 1), zones[timer[timer_id]['id']]['name'],
                            ...
                            if CheckPartner(userid):
                                if not timer[timer_id]['state'] == 2:
                                    esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou automatic unpartnered with %s' % es.getplayername(
                                        player[steamid]["partner"]))
                                    esc.tell(player[steamid]["partner"],
                                             '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou automatic unpartnered with %s' % es.getplayername(userid))
                                    player[steamid]["partner"] = None
...
def tp_end(userid):
    steamid = es.getplayersteamid(userid)
    if "Map" in commands:
        if not zones[str(commands["Map"])]['location_3'] == "none" and not zones[str(commands["Map"])]['location_4'] == "none" :
...
def tp_bend(userid, text, steamid, name):
    ...
    if number == 0:
        if "Bonus 1" in commands:
            if not zones[str(commands["Bonus 1"])]['location_3'] == "none" and not zones[str(commands["Bonus 1"])]['location_4'] == "none":
            ...
            if not zones[str(commands["Bonus %s" % number])]['location_3'] == "none" and not zones[str(commands["Bonus %s" %number])][
                'location_4'] == "none":
        ...
        if not ply.isdead:
            if steamid in timer:
                if steamid in player:
                    ...
                    if player[steamid]['disabled'] == 0:
                        for id in zones:
                            if zones[id]['location_1'] != "none" and zones[id]['location_2'] != "none" and vecmath.isbetweenRect(location, zones[id]['location_1'],zones[id]['location_2']):
                            ...
                            # SOLO TIMER
                                ...
                                player[steamid]['id'] = timer[steamid]['id']
                                string = '(%s)\n \nMode: %s' % (
                                TimeFormat(float(time.time()) - float(timer[steamid]['time']), 1), timer[steamid]['style'])
...
def GetWRPersonal(map, style, type):
    ...
    wr = mysql.query.fetchone("SELECT steamid, time, id FROM completed_personal WHERE map='%s' AND style='%s' AND type='%s' AND season='%s' ORDER BY time ASC",
...
def SetRankedPoints(id, map, steamid, type, personal=False):
        index = 1
        ranked_position = mysql.query.fetchall("SELECT id FROM completed WHERE map='%s' AND type='%s' AND season=%i ORDER BY time ASC", (map, type, c_season))
        if bool(ranked_position):
            for pos in ranked_position:
                if str(pos[0]) == str(id):
                    points = mysql.query.fetchall("SELECT name, points FROM zones WHERE map='%s' AND type='timer'", (map,))

                    for object in points:
                        if object[0] == type:
                            points_result = int((index ** -0.5) * 75) + int(object[1]) - index
                            mysql.query.execute("UPDATE completed SET points='%s' WHERE id=%i", (points_result, id))
                index += 1
...
def GetRecordExists(steamid, map, type, personal=False):
    index = 0
    if not personal:
        records = mysql.query.fetchone("SELECT id, map, steamid, steamid_partner FROM completed WHERE (steamid='%s' OR steamid_partner='%s') AND map='%s' AND type='%s' AND season=%i ORDER BY ASC",
...
def UpdateRankedPoints(steamid, personal=False):
    points = 0
    records = mysql.query.fetchall("SELECT map, points FROM completed WHERE season=%i AND (steamid='%s' OR steamid_partner='%s') ORDER BY time ASC", (c_season, steamid, steamid))
    for object in records:
        points += object[1]
    mysql.query.execute("UPDATE stats SET points='%s' WHERE steamid='%s'", (points, steamid))

def UpdateRankedPointsAll(personal=False):
    data = mysql.query.fetchall('SELECT steamid FROM stats')
    for steamid in data:
        points = 0
        map = ""
        style = ""
        records = mysql.query.fetchall("SELECT map, points, style FROM completed WHERE season=%i AND (steamid='%s' OR steamid_partner='%s') ORDER BY map, time ASC", (c_season, steamid[0], steamid[0]))
        for object in records:
            ...
            style = object[2]
            map = object[0]
        mysql.query.execute("UPDATE stats SET points='%s' WHERE steamid='%s'", (points, steamid[0]))




def UpdateRanks(personal=False):
    records = mysql.query.fetchall("SELECT id, map, steamid, type FROM completed WHERE season=%i" % c_season)
    # Type Map
    if bool(records):
        for data in records:
            # Steamid
...
def EndZone(userid):
    if es.exists("userid", userid):
        ...
        velocity = round(vector((float(es.getplayerprop(userid, 'CBasePlayer.localdata.m_vecVelocity[0]')),
                           ...
                           float(es.getplayerprop(userid, 'CBasePlayer.localdata.m_vecVelocity[2]')))).length(), 2)
            ...
            if timer[steamid]["state"] == 2:
                if timer[steamid]['id'] == id:
                    ...
                    data = mysql.query.fetchone(
                        ...
                        (steamid, str(current_map), timer[steamid]['style'], zones[timer[steamid]['id']]['name'], c_season))
                    ...
                    if bool(data):
                        if data[0] > finish_time:
                            esc.tell(userid, '#255,0,0#255,51,0[#0,137,255 Timer #255,51,0] #snowPersonal time improved by #245,61,0%s' % (
                            ...
                            esc.tell(userid, '#255,0,0#255,51,0[#0,137,255 Timer #255,51,0] #snowNo new personal time...')
                    ...
                    if finish_time >= WR:
                        esc.msg(
                            ...
                            name, zones[timer[steamid]['id']]['name'], TimeFormat(finish_time), timer[steamid]['style'],
                ...
                if timer[timer_id]["id"] == id:
                    ...
                    data = mysql.query.fetchone(
                        ...
                        (steamid, str(current_map), timer[timer_id]['style'], zones[timer[timer_id]['id']]['name'], c_season))
                    ...
                    data = mysql.query.fetchone(
                        ...
                        (steamid_partner, str(current_map), timer[timer_id]['style'], zones[timer[timer_id]['id']]['name']))

                    if bool(data):
                        if data[0] > finish_time:
                            esc.tell(partner, '#255,0,0#255,51,0[#0,137,255 Timer #255,51,0] #snowPersonal time improved by #245,61,0%s' % (
                            TimeFormat(data[0] - finish_time)))
                            mysql.query.execute(
                                "UPDATE completed_personal SET map='%s', steamid='%s', name='%s', country='%s', style='%s', type='%s', time=%f, jumps=%i, flashbangs=%i, date=CURRENT_TIMESTAMP WHERE steamid='%s' AND map='%s' AND style='%s' AND type='%s' AND season=%i",
                                (
                                    ...
                                    str(current_map), timer[timer_id]['style'], zones[timer[timer_id]['id']]['name'], c_season))

                        else:
                            esc.tell(partner, '#255,0,0#255,51,0[#0,137,255 Timer #255,51,0] #snowNo new personal time...')
                        ...
                        mysql.query.execute(
                            "INSERT INTO completed_personal (map, steamid, name, country, style, type, time, jumps, flashbangs, date, points, season) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', %f, %i, %i, CURRENT_TIMESTAMP, %i, %i)" %
                            (
                                str(current_map), steamid_partner, name_partner, country_partner, timer[timer_id]['style'],
                                zones[timer[timer_id]['id']]['name'], finish_time, timer[timer_id]['jumps'],
                                timer[timer_id]['flashbangs'], 0, c_season))

                    """ DATA TO COMPLETED """

                    check = mysql.query.fetchone("SELECT steamid, steamid_partner, time, id FROM completed WHERE steamid='%s' AND steamid_partner='%s' AND style='%s' AND type='%s' AND map='%s' AND season=%s",
                        (
                    ...
                    if not bool(check):
                        check = mysql.query.fetchone(
                            ...
                            (
                                steamid, steamid_partner, timer[timer_id]['style'], zones[timer[timer_id]['id']]['name'],
                                str(current_map), c_season))

                    if bool(check):
                        if check[2] > finish_time:
                            esc.tell(userid,
                                     "#pinkTeam record improved by #245,0,61%s!" % (TimeFormat((check[2] - finish_time))))
                            esc.tell(partner,
                                     "#pinkTeam record improved by #245,0,61%s!" % (TimeFormat((check[2] - finish_time))))
                            mysql.query.execute(
                                ...
                                (
                                    ...
                                    finish_time, timer[timer_id]["jumps"], timer[timer_id]["flashbangs"], check[3], c_season))
                        else:
                        ...
                        mysql.query.execute(
                            ...
                            (
                                str(current_map), steamid, steamid_partner, name, name_partner, country, country_partner,
                                timer[timer_id]["style"], zones[timer[timer_id]['id']]['name'],
...
def GetPersonalWR(steamid, style, map, type):
        p_time = mysql.query.fetchone("SELECT time, id FROM completed_personal WHERE steamid='%s' AND map='%s' AND style='%s' AND type='%s' AND season=%s",(steamid, map, style, type, c_season))
        if bool(p_time):
            return(p_time[0])
        else: return(0)
...
def GetWRName2(steamid, style, map, type, personal=False):
        if personal:
            ...
            data = mysql.query.fetchone("SELECT name, name_partner FROM completed WHERE map='%s' AND style='%s' AND type='%s' AND season=%s ORDER BY time ASC",(map, style, type, c_season))
                        ...
                        if player[steamid]["id"] in zones:
                            if zones[str(player[steamid]["id"])]['type'] == "timer":
                                ...
                                if zones[player[steamid]["id"]]["partner_needed"] == 1:
                                    ...
                                    string += "\nChallenge: Solo"
                                if GetPersonalWR(steamid, "Normal", str(current_map), zones[player[steamid]["id"]]["name"]) != 0:
                                    string += "\n\n\nPersonal best: %s" % TimeFormat(GetPersonalWR(steamid, "Normal", str(current_map), zones[player[steamid]["id"]]["name"]))
                                else:
                                    string += "\n\nPersonal best: None"

                                if zones[player[steamid]["id"]]["partner_needed"] == 1:
                                    WrName = GetWRName2(steamid, "Normal", str(current_map), zones[player[steamid]["id"]]["name"])
                                    if WrName != 0:
                                        string += "\n\nWR: %s and %s" % (WrName[0], WrName[1])
                                    else:
                                        string += "\n\nWR: "

                                    WrName = GetWRName2(steamid, "Sideways", str(current_map), zones[player[steamid]["id"]]["name"])
                                    if WrName != 0:
                                        string += "\nSW: %s and %s" % (WrName[0], WrName[1])
                                    else:
                                        string += "\nSW: "

                                    WrName = GetWRName2(steamid, "Half-Sideways", str(current_map), zones[player[steamid]["id"]]["name"])
                                    if WrName != 0:
                                        string += "\nHSW: %s and %s" % (WrName[0], WrName[1])
                                    else:
                                        string += "\nHSW: "

                                    WrName = GetWRName2(steamid, "W-Only", str(current_map), zones[player[steamid]["id"]]["name"])
                                    if WrName != 0:
                                    ...
                                    WrName = GetWRName2(steamid, "Normal", str(current_map),zones[player[steamid]["id"]]["name"], True)
                                    if WrName != 0:
                                        string += "\n\nWR: %s" % WrName
                                    else:
                                        string += "\n\nWR: "

                                    WrName = GetWRName2(steamid, "Sideways", str(current_map),zones[player[steamid]["id"]]["name"], True)
                                    if WrName != 0:
                                        string += "\nSW: %s" % WrName
                                    else:
                                        string += "\nSW: "

                                    WrName = GetWRName2(steamid, "Half-Sideways", str(current_map),zones[player[steamid]["id"]]["name"], True)
                                    if WrName != 0:
                                        string += "\nHSW: %s" % WrName
                                    else:
                                        string += "\nHSW: "

                                    WrName = GetWRName2(steamid, "W-Only", str(current_map),zones[player[steamid]["id"]]["name"], True)
                                    if WrName != 0:
                                        string += "\nW-Only: %s" % WrName
                                    else:
                                        string += "\nW-Only: "
...
class Menu:
    ...
    def start_menu(self, userid):
        ...
        else: esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou are not authorized to run this #yellowcommand!')
    ...
    def zones_modify(self, userid, id):
        ...
        TimerSolo_Stop(userid)
        TimerPartner_Stop(userid)
        data = mysql.query.fetchone("SELECT id, type, name, tier, points, partner_needed FROM zones WHERE id='%s'", (id,))
    ...
    def zones_modify_select(self, userid, choice, popupid):
        ...
        if int(choice) == 1:
            ...
            esc.tell(userid, "#255,51,0[#0,137,255 Timer #255,51,0] #snowWhat should this zone do? If you choose Timer it requires an end zone!")
            ...
            if data[5] == 1:
                ...
                esc.tell(userid, "#255,51,0[#0,137,255 Timer #255,51,0] #snowPartner needed is now set to: #245,61,0Yes")
            ...
            esc.tell(userid, "#255,51,0[#0,137,255 Timer #255,51,0] #snowYou have now added teleport location for this zone")
        ...
        if auth['vaild']:
            if auth_zones() == "True":
                if auth['vaild'] == steamid:
                    ...
                    esc.tell(userid,
                             "#255,51,0[#0,137,255 Timer #255,51,0] #snowYou have a created zone #yellow%s " % text[4:int(
                                 get_len)])

                else:
                    esc.tell(userid, "#255,51,0[#0,137,255 Timer #255,51,0] #snowA zone is already in progress, you are not authorized to make changes right now")
            else:
                esc.tell(userid,
            ...
            esc.tell(userid, "#255,51,0[#0,137,255 Timer #255,51,0] #snowThere are no #yellowzones#snow is in progress!")
    ...
    def partner_menu_select(self, userid, choice, popupid):
        ...
        if str(player[steamid_partner]["partner"]) == str(userid):
            ...
            esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou are now partnered with %s' % es .getplayername(choice))
            esc.tell(choice, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou are now partnered with %s' % name)
        else:
            esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowWaiting for the other player... will select you..')
            esc.tell(choice,
    ...
    def wrtop(self, steamid, userid, type, style, map_choose, personal=False):
        ...
        if personal:
            ...
            partner = mysql.query.fetchall(
                "SELECT time, name, name_partner, points FROM completed WHERE map='%s' AND type='%s' AND style='%s' AND season='%s' ORDER BY time ASC LIMIT 100",(str(map_choose), str(type), str(style), c_season))
            for item in partner:
                index += 1
                full_top.addoption(index,
                                   ("#%s: %s - ( %s | %s ) [points: %s]" % (index, TimeFormat(float(item[0])), item[1], item[2],item[3])))
    ...
    def wrtop_season(self, season, steamid, userid, type, style, map_choose, personal=False):
        ...
        if personal:
            ...
            partner = mysql.query.fetchall(
                "SELECT time, name, name_partner, points FROM completed WHERE map='%s' AND type='%s' AND style='%s' AND season='%s' ORDER BY time ASC LIMIT 100",(str(map_choose), str(type), str(style), season))
            for item in partner:
                index += 1
                full_top.addoption(index,
                                   ("#%s: %s - ( %s | %s ) [points: %s]" % (index, TimeFormat(float(item[0])), item[1], item[2], item[3])))
    ...
    def wrselect(self, steamid, userid, type, style, map_choose, personal=False):
        ...
        data = mysql.query.fetchone("SELECT partner_needed, points FROM zones WHERE map='%s' AND name='%s'", (map_choose, type))
        if bool(data):
            if int(data[0]) == 0:
                personal = True
    ...
    def bwrselect(self, steamid, userid, type, style, map_choose, personal=False):
        ...
        full_top.c_pagesep = " "
        full_top.enablekeys = "0123490"
        data = mysql.query.fetchall("SELECT name, points FROM zones WHERE map='%s' AND type='timer' ORDER BY name ASC", (map_choose,))
        if bool(data):
            for item in data:
                if not item[0] == "Map":
    ...
    def ptop(self, steamid, userid, type, style, map_choose):
        ...
        full_top.settitle("Trikz Timer [Top 100]\n%s (%s Timer, %s)\nSeason %s\nPersonal world records.." % (map_choose, type, style, c_season))
        # full_top.submenu(10, 'Main Guild')
        full_top.c_beginsep = " "
        full_top.c_pagesep = " "
        full_top.enablekeys = "12098"
    ...
    def ptop_season(self, season, steamid, userid, type, style, map_choose):
        index = 0
        full_top = popuplib.easymenu(steamid + 'Ptop', None, self.ptop_select)
        full_top.settitle("Trikz Timer [Top 100]\n%s (%s Timer, %s)\nSeason %s\nPersonal world records..\n - OLD RECORDS -" % (map_choose, type, style, c_season))
        # full_top.submenu(10, 'Main Guild')
        full_top.c_beginsep = " "
        full_top.c_pagesep = " "
        full_top.enablekeys = "098"
    ...
    def btop(self, steamid, userid, type, style, map_choose):
        ...
        partner = mysql.query.fetchall("SELECT steamid, time, name FROM completed_personal WHERE map='%s' AND type='%s' AND style='%s' AND season='%s' ORDER BY time ASC LIMIT 100",
            (str(map_choose), str(type), str(style), c_season))
        for item in partner:
            index += 1
    ...
    def btop_season(self, steamid, userid, type, style, map_choose):
        index = 0
        full_top = popuplib.easymenu(steamid + 'btop', None, self.btop_select)
        full_top.settitle("Trikz Timer [Top 100]\n%s (%s Timer, %s)\nPersonal world records..\n- OLD RECORDS -" % (map_choose, type, style))
        # full_top.submenu(10, 'Main Guild')
        full_top.c_beginsep = " "
        full_top.c_pagesep = " "
        full_top.enablekeys = "098"
        ...
        if int(choice) == 1:
            if id in timer and CheckPartner(userid):
                if not timer[id]["style"] == "Normal":
                    ...
                    esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou have changed the mode to #tomatoNormal')
                    esc.tell(player[steamid]["partner"],
            ...
            if id in timer and CheckPartner(userid):
                if not timer[id]["style"] == "Sideways":
                    ...
                    esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou have changed the mode to #tomatoSideways')
                    esc.tell(player[steamid]["partner"],
            ...
            if id in timer and CheckPartner(userid):
                if not timer[id]["style"] == "Half-Sideways":
                    ...
                    esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou have changed the mode to #tomatoHalf-Sideways')
                    esc.tell(player[steamid]["partner"],
            ...
            if id in timer and CheckPartner(userid):
                if not timer[id]["style"] == "W-Only":
                    ...
                    esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou have changed the mode to #tomatoW-Only')
                    esc.tell(player[steamid]["partner"],
    ...
    def settings(self, userid):
        ...
        else:info.addline('->1. Timer [Enabled]')
        info.addline(' ')
        ...
        else:info.addline('->3. Text Middle-Bottom [Disabled]')

        if player[steamid]["settings_jumps"] == 1:
            info.addline('->4. Show Jumps [On]')
        else:info.addline('->4. Show Jumps [Off]')
        if player[steamid]["settings_flashbangs"] == 1:
            info.addline('->5. Show Flashbangs [On]')
        else:info.addline('->5. Show Flashbangs [Off]')
        if player[steamid]["settings_strafes"] == 1:
            info.addline('->6. Show Strafes [On]')
        else:info.addline('->6. Show Strafes [Off]')
        if player[steamid]["settings_speedmeter"] == 1:
            info.addline('->7. Show SpeedMeter [On]')
        else:info.addline('->7. Show SpeedMeter [Off]')
        if player[steamid]["settings_macro"] == 1:
            info.addline('->8. DOLGs Macro [On]')
        else: info.addline('->8. DOLGs Macro [Off]')
        info.addline(' ')
        info.addline(' ')
        info.addline('0. Exit')
    ...
    def settings_select(self, userid, choice, popupid):
        ...
        if int(choice) == 1:
            ...
            if player[steamid]["disabled_keyhint"] == 0:
                ...
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYour #245,61,0keyhint #snowwill no longer be #yellowdisplayed!')
                ...
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYour #245,61,0keyhint #snowwill only show #yellowspectators')
            else:

                player[steamid]["disabled_keyhint"] = 0
                client_pref[steamid]["disabled_keyhint"] = 0

                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYour #245,61,0keyhint #snowwill now #yellowshow!')
            ...
            if player[steamid]["settings_hudhint"] == 0:
                ...
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou can now see your #245,61,0hudhint #snowagain!')
            else:

                player[steamid]["settings_hudhint"] = 0
                client_pref[steamid]["settings_hudhint"] = 0
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYour #245,61,0hudhint #snowwill no longer be #yellowdisplayed!')

        elif int(choice) == 4:
            if player[steamid]["settings_jumps"] == 0:
                player[steamid]["settings_jumps"] = 1
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Jumps #snowwill now be displayed in your #yellowhudhint')
            else:
                player[steamid]["settings_jumps"] = 0
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Jumps #snowwill no longer be displayed in your #yellowhudhint')

        elif int(choice) == 5:
            if player[steamid]["settings_flashbangs"] == 0:
                player[steamid]["settings_flashbangs"] = 1
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Flashbangs #snowwill now be displayed in your #yellowhudhint')
            else:
                player[steamid]["settings_flashbangs"] = 0
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Flashbangs #snowwill no longer be displayed in your #yellowhudhint')

        elif int(choice) == 6:
            if player[steamid]["settings_strafes"] == 0:
                player[steamid]["settings_strafes"] = 1
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Strafes #snowwill now be displayed in your #yellowhudhint')
            else:
                player[steamid]["settings_strafes"] = 0
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Strafes #snowwill no longer be displayed in your #yellowhudhint')

        elif int(choice) == 7:
            if player[steamid]["settings_speedmeter"] == 0:
                player[steamid]["settings_speedmeter"] = 1
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Speedmeter #snowwill now be displayed in your #yellowhudhint')
            else:
                player[steamid]["settings_speedmeter"] = 0
                esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Speedmeter #snowwill no longer be displayed in your #yellowhudhint')
            ...
            if steamid == "[U:1:27015871]" or steamid == "[U:1:42504738]" or steamid == "[U:1:11635]":
                if client_pref[steamid]["settings_macro"] == 0:
                    ...
                    esc.tell(userid,'#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Your custom macro has been #yellowenabled!')
                else:
                    client_pref[steamid]["settings_macro"] = 0
                    player[steamid]["settings_macro"] = 0
                    es.cexec(userid, 'sm_macro')
                    esc.tell(userid,'#255,51,0[#0,137,255 Timer #255,51,0] #245,61,0Your custom macro has been #yellowdisabled!')
...
class Notify:
    ...
    def challenge_points(self, userid, text, steamid, name):
            ...
            zone_details = mysql.query.fetchall("SELECT name, points FROM zones WHERE map='%s' AND type='timer'", (map_choose,))
    ...
    def unpartner(self, userid):
        steamid = es.getplayersteamid(userid)
        if CheckPartner(userid):
            esc.tell(userid, '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou are no longer partnered with#yellow %s' % es.getplayername(
                player[steamid]["partner"]))
            esc.tell(player[steamid]["partner"],
                     '#255,51,0[#0,137,255 Timer #255,51,0] #snowYou are no longer partnered with#yellow %s' % es.getplayername(userid))
            player[steamid]["partner"] = None
    ...
    def register(self, steamid, userid):
        ...
        if len(tournament["queue"]) == 0:
            ...
            for team in tournament["queue"]:
                ...
                index += 30
                if tplayer["steamid"]:
                    full_top.addoption("signup", ("%s | %s (in %s mins)" % (tplayer["name"], tplayer_2["name"], index)), state=False)
                else:
                    full_top.addoption("signup", ("%s | %s (Finished)" % (tplayer["name"], tplayer_2["name"])),state=False)
        ...
        if choice == "signup":
            if CheckPartner(userid):
                ...
                if not self.CheckRegister(steamid) and not self.CheckRegister(steamid_partner):
                    ...
                    tournament["queue"][position_in_queue] = [{'steamid':steamid, 'name':name, 'time':position_in_queue*1800}, {'steamid':steamid_partner,'name':name_partner, 'time':position_in_queue*1800}]
                    esc.tell(userid,'#255,51,0[#0,137,255 Tournament #255,51,0] #snowYou have now signed up with#yellow %s #snow(Waiting time is approx: ??)' % es.getplayername(player[steamid]["partner"]))
                    esc.tell(partner_userid, "#255,51,0[#0,137,255 Tournament #255,51,0] #snowYour partner#yellow %s have just signed up for tournament. Good luck!" % name)
                    esc.tell(userid,'#255,51,0[#0,137,255 Tournament #255,51,0] #255,0,0You do not need to be in the server while waiting, just come back when it is your turn!')
                else:
                    esc.tell(userid,
                ...
                esc.tell(userid,'#255,51,0[#0,137,255 Tournament #255,51,0] #snowYou need a partner before you can sign up!#')
    ...
    def teleport(self, userid):
        if tournament["status"] == 1:
            ...
            if tournament["turn"] in tournament["queue"]:
                if steamid == tournament["queue"][tournament["turn"]][0]["steamid"] or steamid == tournament["queue"][tournament["turn"]][1]["steamid"]:
                    return
                else:
                    ...
                    if w_steamid in active_players and not w_steamid_partner in active_players:
                        ...
                        for userid in es.getUseridList():
                            ...
                            if w_steamid == steamid:
                                es.centertell(userid, "You partner is disconnected!\nYour partner has %s mins to connect again!" % (TimeFormat(tournament["disconnect"], None, True)))
                        ...
                        if w_steamid in auto_partner and w_steamid_partner in auto_partner:
                            if player[w_steamid]["timer_id"] == None or player[w_steamid_partner]["timer_id"] == None:
                                ...
                                timer[player[w_steamid_partner]["timer_id"]] = {'partner_needed': 1,
                                                                              ...
                                                                              'state': 0}
                                esc.tell(auto_partner[w_steamid_partner], '#255,51,0[#0,137,255 Tournament #255,51,0] #yellowYou partner has rejoined! You are automatic partned again!')
                                esc.tell(auto_partner[w_steamid],'#255,51,0[#0,137,255 Tournament #255,51,0] #yellowYou partner has rejoined! You are automatic partned again!')
